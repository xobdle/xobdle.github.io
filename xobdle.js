...
function todayStartedKey() {
  return `xobdle-started-${getISTISODate(0)}`;
}

function hasStartedToday() {
  try {
    return localStorage.getItem(todayStartedKey()) === "1";
  } catch (error) {
    console.warn("Could not read today's Xobdle start state:", error);
    return false;
  }
}
...
function markStartedToday() {
  try {
    localStorage.setItem(todayStartedKey(), "1");
  } catch (error) {
    console.warn("Could not save today's Xobdle start state:", error);
  }
}
...
entryYesterdayBtn.addEventListener("click", () => {
  updateYesterdayPopup();
  yesterdayOverlay.classList.remove("hidden");
});

entryHelpBtn.addEventListener("click", openInstructions);
...
function getActivePage() {
  ...
  if (!document.getElementById("entryPage").classList.contains("hidden")) {
    return document.getElementById("entryPage");
  }
...
async function initializeXobdle() {
  ...
  if (data.status === "live") {
    ...
    const entryIsVisible =
      !document.getElementById("entryPage").classList.contains("hidden");

    if (
      !entryIsVisible &&
      !gameOver &&
      !localStorage.getItem("xobdleInstructionsSeen")
    ) {
      openInstructions();
    }
  } else {
